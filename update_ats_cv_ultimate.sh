...
echo "=== Memperbarui cv.html dengan Kontak Berikon, Layout Beda per Tema & Auto-Anchor ==="
...
<head>
  ...
  <title>GAEKS ATS CV Studio | Professional Resume Suite</title>
...
<body class="bg-slate-950 text-slate-100 antialiased min-h-screen flex flex-col justify-between selection:bg-blue-600 selection:text-white">
  ...
  <section id="view-dashboard" class="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 flex-grow">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between pb-6 mb-6 border-b border-slate-800 gap-4">
      <div>
        ...
        <p class="text-xs sm:text-sm text-slate-400 mt-1">Kelola resume Anda, duplikasi untuk variasi posisi, atau buat resume baru.</p>
      </div>
  ...
  <section id="view-editor" class="hidden max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6 flex-grow">
    ...
    <div class="no-print flex flex-col md:flex-row md:items-center justify-between pb-4 mb-5 border-b border-slate-800 gap-4">
      ...
      <!-- KONTROL FITUR: UNDO/REDO, BAHASA & TEMA -->
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
      ...
      <div class="no-print lg:col-span-5 bg-slate-900/90 rounded-2xl border border-slate-800 p-4 sm:p-5 max-h-[85vh] overflow-y-auto space-y-4">
        ...
        <div id="ftab-content-skills" class="hidden space-y-3">
          ...
          <div>
            <label class="block text-[11px] text-slate-400 mb-1">Tools & Platform (Pisahkan koma)</label>
          ...
          <div>
            <label class="block text-[11px] text-slate-400 mb-1">Sertifikasi & Lisensi</label>
      ...
      <!-- PRATINJAU LEMBAR A4 DENGAN 3 VARIAN LAYOUT (7 KOLOM) -->
        ...
        <div id="cv-preview-sheet" class="cv-sheet-font w-full max-w-[780px] bg-white text-slate-800 shadow-2xl rounded-sm p-8 sm:p-12 min-h-[1050px] border border-slate-200">
          ...
          <div id="cv-header-container" class="pb-4 border-b-2 theme-accent-border flex flex-col sm:flex-row items-center sm:items-start gap-5">
            ...
            <div class="flex-grow text-center sm:text-left">
              ...
              <p id="view-title" class="text-sm sm:text-base font-bold text-slate-800 mt-0.5 tracking-wide">Target Posisi</p>

              <!-- BARIS KONTAK BERIKON RINGKAS & PROFESIONAL -->
              <div class="flex flex-wrap items-center gap-x-3.5 gap-y-1.5 text-xs text-slate-600 mt-3 font-medium justify-center sm:justify-start">
                ...
                <!-- LinkedIn -->
                <span id="wrap-linkedin" class="inline-flex items-center gap-1.5">
                  <span class="font-bold text-[11px] text-slate-500 px-1 py-0.2 rounded bg-slate-100 border border-slate-200">in</span>
                  <span id="view-linkedin">linkedin.com/in/...</span>
                </span>
                
                <!-- Portfolio -->
                <span id="wrap-portfolio" class="inline-flex items-center gap-1.5">
          ...
          <!-- 3. EXPERIENCE DENGAN PEMBEDA JABATAN & PERUSAHAAN -->
          <div class="mt-4">
  ...
  <!-- SCRIPT LOGIKA UTAMA -->
  <script>
    // 3 TEMA DENGAN STRUKTUR LAYOUT BERBEDA
    const themePacks = {
    ...
    // KAMUS PENERJEMAHAN ID -> EN KOMPREHENSIF
    ...
    function createDefaultCvObject(title = "CV Baru") {
      return {
        ...
        data: {
          ...
          title: "Head of Operations & Logistics Ecosystem",
          ...
          skills: "Freight Forwarding, Supply Chain Optimization, Customs Clearance (PPJK), Vendor Sourcing, Contract Negotiation, Multimodal Logistics",
          tools: "Dolibarr ERP, SAP S/4HANA, Excel Advanced, SQL, Cloudflare",
          certifications: "Sertifikasi Ahli Kepabeanan (PPJK) — BPPK Kemenkeu\nCertified Supply Chain Manager (CSCM)",
    ...
    // AUTO ANCHOR & FOCUS IMPLEMENTATION
    function addExperience(item = { role: '', company: '', period: '', location: '', desc: '' }) {
      ...
      // Scroll halus dan fokus otomatis
    ...
    function addEducation(item = { degree: '', school: '', period: '', details: '' }) {
      ...
      div.innerHTML = `
        ...
        <div class="grid grid-cols-2 gap-2">
          ...
          <div>
            <label class="block text-[10px] text-slate-400 mb-0.5">Nama Kampus / Sekolah</label>
        ...
        <div class="grid grid-cols-2 gap-2">
          <div>
            <label class="block text-[10px] text-slate-400 mb-0.5">Tahun Kelulusan</label>
          ...
          <div>
            <label class="block text-[10px] text-slate-400 mb-0.5">IPK / Keterangan</label>
    ...
    function renderCV() {
      ...
      // 3. Experience (DENGAN PEMBEDA JABATAN VS PERUSAHAAN & VARIASI LAYOUT)
      ...
      document.querySelectorAll('.exp-item').forEach(el => {
        ...
        if (themeCfg.layout === 'split') {
          // Varian 2: Emerald Split Line (Garis vertikal aksen di kiri)
          expView.innerHTML += `
            <div class="pl-3 border-l-2 theme-accent-border relative">
              ...
              <ul class="text-xs leading-relaxed text-slate-700 mt-1.5 space-y-0.5">${bullets || '<li>Pencapaian kerja...</li>'}</ul>
            </div>
          `;
        } else if (themeCfg.layout === 'classic') {
          // Varian 3: Slate Minimal Classic ATS (Linier rapi tanpa border samping)
          expView.innerHTML += `
            <div>
              ...
              <ul class="text-xs leading-relaxed text-slate-700 mt-1 space-y-0.5">${bullets || '<li>Pencapaian kerja...</li>'}</ul>
            </div>
          `;
        } else {
          // Varian 1: Executive Navy (Default)
          expView.innerHTML += `
            <div>
              <div class="flex flex-wrap justify-between items-baseline gap-1">
                <div>
...
git add cv.html
git commit -m "feat: enhance contact icons, distinct company styling, structural theme layouts and auto-anchor inputs"
