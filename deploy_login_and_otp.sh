...
echo "=== 1. Membuat login.html (Halaman Dedikasi Masuk & Daftar Profesional) ==="
    ...
    <div class="lg:col-span-7 p-8 sm:p-10 flex flex-col justify-between bg-slate-900/60">
      ...
      <div>
        ...
        <div class="space-y-4 mb-5">
          <button type="button" onclick="startGoogleAuth()" class="w-full py-3 px-4 bg-white hover:bg-slate-100 text-slate-900 font-bold text-xs sm:text-sm rounded-xl flex items-center justify-center gap-3 transition shadow-lg shadow-black/30">
            ...
            <span id="label-google-btn">Lanjutkan dengan Akun Google</span>
          </button>
        ...
        <form id="form-login" onsubmit="submitLogin(event)" class="space-y-3.5">
          ...
          <button type="submit" class="w-full py-3 bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs sm:text-sm rounded-xl transition shadow-lg shadow-blue-500/20">
      ...
      <div class="pt-6 border-t border-slate-800 flex justify-between items-center text-xs text-slate-500">
        <a href="index.html" class="hover:text-slate-300 transition">&larr; Kembali ke Beranda</a>
  ...
  <script>
    window.addEventListener('DOMContentLoaded', () => {
      if (window.location.hash.includes('access_token=')) {
        ...
        if (accessToken) {
          showAlert("✓ Berhasil terhubung ke Google! Menyiapkan sesi Anda...", true);
          ...
          .then(profile => {
            if (profile && profile.email) {
              GaeksAuth.processVerifiedGoogleUser(profile.email, profile.name, profile.picture);
              window.location.href = 'cv.html';
            }
          })
          .catch(err => {
            showAlert("Gagal mengambil profil dari Google. Silakan coba lagi.");
          });
          return;
        }
      }
    ...
    function setAuthTab(tab) {
      ...
      const gBtnText = document.getElementById('label-google-btn');
      hideAlert();
      ...
      if (tab === 'login') {
        ...
        gBtnText.innerText = "Lanjutkan dengan Akun Google";
      } else {
        ...
        gBtnText.innerText = "Daftar Cepat dengan Akun Google";
      }
    }
    ...
    function startGoogleAuth() {
      const clientId = "41832472270-6r8iudma1eho6kn3q6rs4rl7b9ank7n4.apps.googleusercontent.com";
      const redirectUri = window.location.origin + '/login.html';
      const googleOAuthUrl = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=token&scope=email%20profile%20openid&prompt=select_account`;
      
      const w = 500, h = 600;
      const left = (window.screen.width / 2) - (w / 2);
      const top = (window.screen.height / 2) - (h / 2);
      const popup = window.open(googleOAuthUrl, "GoogleLoginPopup", `width=${w},height=${h},top=${top},left=${left}`);
    ...
    async function submitOtpVerification() {
      ...
      try {
        ...
        if (data.status === 'success') {
          showAlert("✓ Verifikasi berhasil! Mengalihkan ke studio CV...", true);
          GaeksAuth.loginWithEmail(storedEmail, pass, storedName);
          setTimeout(() => { window.location.href = 'cv.html'; }, 800);
        } else {
...
echo "=== 2. Memperbarui auth.js (Sentral Pengendali Google OAuth & Session) ==="
...
echo "=== 3. Memperbarui api/auth_otp.php (Server Pengirim OTP 6 Digit) ==="
mkdir -p api
cat << 'EOF_OTP' > api/auth_otp.php
<?php
...
echo "=== 4. Memperbarui Rute index.html & cv.html ke login.html ==="
python3 -c '
import os

if os.path.exists("index.html"):
    with open("index.html", "r") as f: idx = f.read()
    idx = idx.replace("onclick=\"openAuthModal()\"", "onclick=\"window.location.href=\x27login.html\x27\"")
    idx = idx.replace("function handleServiceAccess(targetUrl) {", "function handleServiceAccess(targetUrl) {\n      const user = GaeksAuth.getCurrentUser();\n      if (user) { window.location.href = targetUrl; }\n      else { window.location.href = \x27login.html\x27; return; }")
    with open("index.html", "w") as f: f.write(idx)

if os.path.exists("cv.html"):
    with open("cv.html", "r") as f: cv = f.read()
    cv = cv.replace("if (!GaeksAuth.getCurrentUser()) { setTimeout(openAuthModal, 300); }", "if (!GaeksAuth.getCurrentUser()) { window.location.href = \x27login.html\x27; return; }")
    cv = cv.replace("onclick=\"openAuthModal()\"", "onclick=\"window.location.href=\x27login.html\x27\"")
    with open("cv.html", "w") as f: f.write(cv)

print("index.html & cv.html patched successfully.")
'

echo "=== 5. Mem-push pembaruan ke GitHub Repository ==="
git add -A
git commit -m "feat: launch dedicated login.html with Google OAuth and 6-digit OTP email verification" || true
git push origin main --force

echo "=== SELESAI! Buka https://gdp.gaeks.com/login.html di peramban ==="
