...
echo "=== 1. Memperbarui cv.html dengan Data Asli Deny Triawan & 4 Layout ATS Eksekutif ==="
...
<head>
  ...
  <title>GAEKS ATS CV Studio | Deny Triawan Professional Resume</title>
...
<body class="bg-slate-950 text-slate-100 antialiased min-h-screen flex flex-col justify-between selection:bg-blue-600 selection:text-white theme-executive" id="app-body">
  ...
  <section id="view-dashboard" class="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 flex-grow">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between pb-6 mb-6 border-b border-slate-800 gap-4">
      <div>
        ...
        <p class="text-xs sm:text-sm text-slate-400 mt-1">Kelola variasi resume Anda dengan 4 gaya arsitektur layout standar internasional.</p>
      </div>
  ...
  <section id="view-editor" class="hidden max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6 flex-grow">
    ...
    <div class="no-print flex flex-col md:flex-row md:items-center justify-between pb-4 mb-5 border-b border-slate-800 gap-4">
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <!-- 4 PILIHAN GAYA & ARSITEKTUR LAYOUT -->
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
      ...
      <div class="no-print lg:col-span-5 bg-slate-900/90 rounded-2xl border border-slate-800 p-4 sm:p-5 max-h-[85vh] overflow-y-auto space-y-4">
        ...
        <div id="ftab-content-profile" class="space-y-3">
          <div class="flex justify-between items-center pb-2 border-b border-slate-800">
            <span class="text-xs font-bold uppercase text-blue-400">Data Personal Deny Triawan</span>
          ...
          <div>
            <label class="block text-[11px] text-slate-400 mb-1">Target Posisi / Preferred Job</label>
          ...
          <div class="grid grid-cols-2 gap-2">
            <div>
              <label class="block text-[11px] text-slate-400 mb-1">Alamat Lengkap</label>
        ...
        <div id="ftab-content-skills" class="hidden space-y-3">
          <div class="pb-2 border-b border-slate-800">
            <span class="text-xs font-bold uppercase text-blue-400">Keahlian, Tools & Sertifikasi</span>
          </div>
          <div>
            <label class="block text-[11px] text-slate-400 mb-1">Keahlian Supply Chain & ERP (Pisahkan koma)</label>
          ...
          <div>
            <label class="block text-[11px] text-slate-400 mb-1">Tools, Software & Hardware (Pisahkan koma)</label>
          ...
          <div>
            <label class="block text-[11px] text-slate-400 mb-1">Sertifikasi Resmi & Publikasi Ilmiah</label>
      ...
      <!-- PRATINJAU LEMBAR A4 DENGAN 4 ARSITEKTUR LAYOUT -->
      <div class="lg:col-span-7 flex flex-col items-center">
        ...
        <div class="no-print w-full flex justify-between items-center mb-2 px-2 text-xs text-slate-400">
          <div class="flex items-center space-x-2">
            ...
            <span id="label-active-layout" class="text-[10px] text-slate-400 font-semibold">Executive Layout</span>
          </div>
        ...
        <div id="cv-preview-sheet" class="cv-sheet-font w-full max-w-[780px] bg-white text-slate-800 shadow-2xl rounded-sm p-8 sm:p-12 min-h-[1050px] border border-slate-200">
          ...
          <!-- RENDER KONTEN AKAN DILAKUKAN OLEH renderCV() BERDASARKAN LAYOUT -->
  ...
  <script>
    ...
    let historyStack = [];
    let historyIndex = -1;
    const STORAGE_KEY = 'gaeks_cv_master_list_v3';

    // 100% DATA RESMI DARI FILE CV DENY TRIAWAN
    function getDenyTriawanOfficialData() {
      return {
        ...
        data: {
          ...
          skills: "Supply Chain Management, Freight Forwarding, Customs Clearance (PIB, PEB, Manifest, BL, COO), Odoo ERP Implementation, Business Process Mapping (IBM/COBIT), Vendor Contract Negotiation, ISO 31000 Risk Management, KPI Development",
          ...
          educations: [
            ...
            {
              degree: "Pelatihan Tingkat Lanjut",
              school: "Oracle Academy & Microsoft Partner",
              period: "2016 - 2018",
              details: "Sertifikasi SQL Database Programming (Oracle Academy) & Sertifikasi Microsoft Office Desktop Mastery"
            }
          ]
        }
      };
    ...
    function createNewCV() {
      ...
      newCv.title = "Deny Triawan - CV (Baru)";
    ...
    function extractFormData() {
      ...
      document.querySelectorAll('.exp-item').forEach(el => {
        exps.push({
          ...
          sector: el.querySelector('.exp-sector') ? el.querySelector('.exp-sector').value : 'Freight Forwarding',
    ...
    function addExperience(item = { role: '', company: '', period: '', location: '', sector: '', desc: '', accomplishment: '' }) {
      ...
      div.innerHTML = `
        ...
        <div class="grid grid-cols-3 gap-2">
          <div>
            <label class="block text-[10px] text-slate-400 mb-0.5">Periode (mis: May 2024 - Present)</label>
    ...
    // RENDER SHEET UTAMA BERDASARKAN 4 LAYOUT
    function renderCV() {
      ...
      document.querySelectorAll('.exp-item').forEach(el => {
        ...
        if (activeLayout === 'europass') {
          // GAYA 1: EUROPASS MODERN (Dua kolom linier: Tanggal di kiri, Detail di kanan)
          experiencesHTML += `
            <div class="grid grid-cols-12 gap-3 mb-4 pb-3 border-b border-slate-100 last:border-none">
              <div class="col-span-3 text-[11px] font-bold accent-text pt-0.5">
                ${per}
              </div>
          ...
          // GAYA 2: TECH / SOLUTION ARCHITECT (Card pill accents & clean split line)
          experiencesHTML += `
          ...
          // GAYA 3: HARVARD MINIMALIST (Garis tipis, hierarki klasik, tanpa kotak mencolok)
          experiencesHTML += `
          ...
          // GAYA 4: EXECUTIVE CORPORATE (Default)
          experiencesHTML += `
            <div class="mb-4">
              <div class="flex flex-wrap justify-between items-baseline gap-1">
                <div>
      ...
      // ASSEMBLE HTML BERDASARKAN TEMA
      root.innerHTML = `
        <!-- HEADER DENGAN DATA RESMI DENY TRIAWAN -->
        <div class="pb-4 border-b-2 accent-border flex flex-col sm:flex-row items-center sm:items-start gap-4">
          ...
          <div class="flex-grow text-center ${hasPhoto ? 'sm:text-left' : 'sm:text-left'}">
            ...
            <!-- CONTACT STRIP DENGAN IKON PROFESIONAL -->
            <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-600 mt-2 font-medium justify-center sm:justify-start">
...
echo "=== 2. Mem-push pembaruan resmi CV Deny Triawan ke GitHub ==="
git add cv.html
git commit -m "feat: adopt 100% authentic Deny Triawan Europass data with 4 distinct ATS layout architectures"
