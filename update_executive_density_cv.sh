...
echo "=== Memperbarui cv.html dengan Standar Eksekutif High-Density (Bebas Elemen AI) ==="
...
<head>
  ...
  <title>GAEKS ATS CV Studio | Executive High-Density Resume</title>
  ...
  <style>
    ...
    /* VARIAN ARSITEKTUR EKSEKUTIF REALISTIS (BEBAS ELEMEN AI) */
    .style-traditional {
    ...
    .heading-accent { color: var(--heading-color) !important; }
    .rule-accent { border-color: var(--rule-color) !important; }

    ::-webkit-scrollbar { width: 5px; height: 5px; }
    ::-webkit-scrollbar-track { background: #090d16; }
    ::-webkit-scrollbar-thumb { background: #1e293b; border-radius: 4px; }

    /* CSS CETAK A4 PRESISI ATS (ANTI-TERPOTONG & EFISIENSI RUANG PENUH) */
    @media print {
...
<body class="bg-slate-950 text-slate-100 antialiased min-h-screen flex flex-col justify-between selection:bg-blue-600 selection:text-white style-traditional" id="main-body">
  ...
  <section id="view-editor" class="hidden max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6 flex-grow">
    ...
    <div class="no-print flex flex-col md:flex-row md:items-center justify-between pb-4 mb-5 border-b border-slate-800 gap-4">
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <!-- 4 TEMA ARSITEKTUR EKSEKUTIF MURNI -->
        <div class="flex items-center space-x-1 bg-slate-900 p-1 rounded-lg border border-slate-800 text-xs font-semibold">
          <button onclick="setThemeLayout('traditional')" id="btn-layout-traditional" class="px-2.5 py-1 rounded bg-blue-950 text-blue-300 border border-blue-600">Wall Street Classic</button>
          <button onclick="setThemeLayout('modern-corporate')" id="btn-layout-modern-corporate" class="px-2.5 py-1 rounded text-slate-400 hover:text-white">Corporate Navy</button>
          <button onclick="setThemeLayout('slate-minimal')" id="btn-layout-slate-minimal" class="px-2.5 py-1 rounded text-slate-400 hover:text-white">Harvard Minimal</button>
          <button onclick="setThemeLayout('compact-serif')" id="btn-layout-compact-serif" class="px-2.5 py-1 rounded text-slate-400 hover:text-white">Executive Serif</button>
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
      ...
      <!-- PRATINJAU LEMBAR A4 ATS DENGAN STRUKTUR DENSITAS TINGGI (7 KOLOM) -->
      <div class="lg:col-span-7 flex flex-col items-center">
        ...
        <div class="no-print w-full flex justify-between items-center mb-2 px-2 text-xs text-slate-400">
          <div class="flex items-center space-x-2">
            ...
            <span id="label-active-layout" class="text-[10px] text-slate-400 font-semibold">Wall Street Classic</span>
          </div>
  ...
  <script>
    ...
    let historyStack = [];
    let historyIndex = -1;
    const STORAGE_KEY = 'gaeks_cv_master_list_v9';
    ...
    function loadFormData(d) {
      ...
      setThemeLayout(d.layout || 'traditional');
    }
    ...
    function addExperience(item = { role: '', company: '', period: '', location: '', desc: '', accomplishment: '' }) {
      ...
      div.innerHTML = `
        ...
        <div>
          ...
          <textarea rows="3" placeholder="${currentLang === 'en' ? 'Start bullets with strong action verbs (e.g. Directed, Spearheaded)...' : 'Gunakan baris baru untuk tiap poin pencapaian...'}" oninput="handleInputChange()" class="exp-desc w-full px-2.5 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded text-white focus:border-blue-500 focus:outline-none">${item.desc}</textarea>
        </div>
        <div>
          ...
          <textarea rows="2" placeholder="${currentLang === 'en' ? 'Describe the most significant result or high-stakes project...' : 'Proyek penting atau angka pencapaian strategis...'}" oninput="handleInputChange()" class="exp-acc w-full px-2.5 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded text-white focus:border-blue-500 focus:outline-none">${item.accomplishment || ''}</textarea>
        </div>
      `;
    ...
    function setThemeLayout(name) {
      ...
      const labels = {
        traditional: "Wall Street Classic",
        "modern-corporate": "Corporate Navy",
        "slate-minimal": "Harvard Minimal",
        "compact-serif": "Executive Serif"
      };
      document.getElementById('label-active-layout').innerText = labels[name] || name;

      ['traditional', 'modern-corporate', 'slate-minimal', 'compact-serif'].forEach(t => {
    ...
    function highlightNumbers(text) {
      if (!text) return '';
      return text.replace(/(\b\d+(\.\d+)?%\b|\b(Rp|USD|\$)\s?[\d.,]+(\s?(juta|miliar|k|m))?|\b\d+x\b|\b\d{2,}\b|\b100\sTransjakarta\sbuses\b|\b100\sbus\sTransjakarta\b)/gi, '<strong class="font-semibold text-slate-900">$1</strong>');
    }

    // RENDER SHEET UTAMA (STANDAR DENSITAS EKSEKUTIF REALISTIS - BEBAS GAYA AI)
    function renderCV() {
      ...
      const title = applyTranslation(document.getElementById('input-title').value) || (currentLang === 'en' ? 'Target Position / Executive Role' : 'Target Posisi / Profesi');
      ...
      // KONTAK MINIMALIS BERSIH (SEPARATOR GARIS PIPA HALUS)
      const contactItems = [];
      ...
      const separatorHTML = `<span class="text-slate-300 font-normal mx-1 select-none">|</span>`;
      const contactLine = contactItems.length > 0 
        ? `<div class="flex flex-wrap items-center justify-center sm:justify-start gap-y-1 text-[11px] text-slate-600 mt-1.5 font-normal">${contactItems.join(separatorHTML)}</div>`
        : '';

      // GENERATOR JUDUL KATEGORI (GARIS HALUS TUNGGAL, BEBAS GAYA AI)
      function renderCategoryHeader(titleText) {
        return `
          <div class="cv-section-title border-b-2 rule-accent pb-0.5 mb-2 mt-4">
            <h2 class="text-[11.5px] font-bold uppercase tracking-wider heading-accent font-sans">${titleText}</h2>
          </div>
        `;
      }

      // EXPERIENCES: DENSITAS TINGGI, 2 BARIS SEJAJAR (JABATAN-PERIODE, PERUSAHAAN-LOKASI)
      let experiencesHTML = '';
      ...
      if (expElements.length === 0) {
        ...
        expElements.forEach(el => {
          ...
          // MAJOR ACCOMPLISHMENT SBG POIN BERBOBOT TINGGI (BUKAN KOTAK AI)
          const accPoint = acc.trim() ? `
            <li class="ml-4 list-disc text-slate-900 font-medium leading-normal pt-0.5">
              <strong class="font-semibold text-slate-950 underline decoration-slate-300 decoration-1 underline-offset-2">${lbl.majorAccomplishmentLabel}:</strong> 
              <span class="text-slate-800">${highlightNumbers(acc)}</span>
            </li>
          ` : '';

          experiencesHTML += `
            <div class="cv-entry-block mb-3 last:mb-1">
              <!-- BARIS 1: JABATAN (KIRI) & PERIODE (KANAN) -->
              <div class="flex justify-between items-baseline text-xs">
                <span class="font-bold text-slate-950 text-[12.5px] tracking-tight">${role || (currentLang === 'en' ? 'Position' : 'Posisi')}</span>
                <span class="font-semibold text-slate-700 text-[11px] shrink-0">${per}</span>
              </div>
              <!-- BARIS 2: PERUSAHAAN (KIRI) & LOKASI (KANAN) -->
              <div class="flex justify-between items-baseline text-[11px] text-slate-600 mb-1">
                <span class="font-medium italic text-slate-800">${comp || (currentLang === 'en' ? 'Organization' : 'Perusahaan')}</span>
                <span class="text-slate-500 italic shrink-0">${itemLoc}</span>
              </div>
              <!-- BULLETS POIN PADAT -->
              <ul class="text-[11px] space-y-0.5 mt-0.5 text-justify">${bullets}${accPoint}</ul>
            </div>
          `;
        });
      }

      // EDUCATIONS: DENSITAS TINGGI
      let educationsHTML = '';
      ...
      if (eduElements.length === 0) {
        ...
        eduElements.forEach(el => {
          ...
          educationsHTML += `
            <div class="cv-entry-block mb-2 last:mb-0.5 text-xs">
      ...
      // SKILLS: COMPACT INLINE CATEGORY (BUKAN BALON AI)
      let skillsGroupHTML = '';
      document.querySelectorAll('.skill-group-item').forEach(el => {
      ...
      if (hasPhoto) {
        headerHTML = `
          <div class="pb-3 border-b-2 rule-accent flex items-center gap-4">
            <img class="w-16 h-16 sm:w-20 sm:h-20 rounded-lg object-cover border border-slate-300 shadow-sm shrink-0" src="${photoData}" alt="${name}" />
            <div class="flex-grow">
              <h1 class="text-2xl sm:text-3xl font-extrabold uppercase tracking-tight heading-accent leading-none">${name}</h1>
              <p class="text-xs sm:text-[13px] font-semibold text-slate-700 mt-1 tracking-wide">${title}</p>
              ${contactLine}
            </div>
          </div>
        `;
        ...
        headerHTML = `
          <div class="pb-3 border-b-2 rule-accent text-center sm:text-left">
            <h1 class="text-2xl sm:text-3xl font-extrabold uppercase tracking-tight heading-accent leading-none">${name}</h1>
            <p class="text-xs sm:text-[13px] font-semibold text-slate-700 mt-1 tracking-wide">${title}</p>
            ${contactLine}
          </div>
      ...
      root.innerHTML = `
        ...
        ${(langRaw.trim() || pubRaw.trim()) ? `
          <div class="mt-3">
            ${renderCategoryHeader(currentLang === 'en' ? 'ADDITIONAL INFORMATION' : 'INFORMASI TAMBAHAN')}
...
echo "=== Mem-push pembaruan Executive High-Density ke GitHub ==="
git add cv.html
git commit -m "feat: complete overhaul to high-density authentic executive resume standard without AI fluff"
