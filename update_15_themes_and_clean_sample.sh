...
echo "=== Memperbarui cv.html: Hapus Sampel Deny Triawan & Terapkan 15 Tema (5 Free + 10 PRO Locked) ==="
...
<head>
  ...
  <style>
    ...
    /* ================= 15 ARSITEKTUR TEMA VISUAL ================= */
    /* 1. Prestige Executive (FREE) */
    .style-prestige {
      --accent-heading: #0f172a;
      --accent-rule: #1e293b;
      --sheet-bg: #ffffff;
    }
    /* 2. Modern Corporate (FREE) */
    .style-modern-corporate {
      --accent-heading: #1e3a8a;
      --accent-rule: #1e3a8a;
      --sheet-bg: #ffffff;
    }
    /* 3. Pure Classic ATS (FREE) */
    .style-pure-ats {
    ...
    /* 5. Silicon Tech (FREE) */
    .style-silicon-tech {
      --accent-heading: #047857;
      --accent-rule: #10b981;
      --sheet-bg: #ffffff;
    }
    /* 6. Emerald Horizon (PRO) */
    .style-emerald-horizon {
      --accent-heading: #065f46;
      --accent-rule: #059669;
      --sheet-bg: #f6fbf8;
    }
    /* 7. Crimson Elegance (PRO) */
    .style-crimson-elegance {
      --accent-heading: #881337;
      --accent-rule: #be123c;
      --sheet-bg: #fdf8f9;
    }
    /* 8. Obsidian Minimal (PRO) */
    .style-obsidian-minimal {
      --accent-heading: #090d16;
      --accent-rule: #090d16;
      --sheet-bg: #ffffff;
    }
    /* 9. Oxford Two-Tone (PRO) */
    .style-oxford-navy {
      --accent-heading: #0f172a;
      --accent-rule: #1e293b;
      --sheet-bg: #ffffff;
    }
    /* 10. Swiss Editorial (PRO) */
    .style-swiss-editorial {
      --accent-heading: #000000;
      --accent-rule: #000000;
      --sheet-bg: #ffffff;
    }
    /* 11. Imperial Gold (PRO) */
    .style-imperial-gold {
      --accent-heading: #92400e;
      --accent-rule: #d97706;
      --sheet-bg: #fdfbf7;
    }
    /* 12. Azure Blue (PRO) */
    .style-azure-sky {
      --accent-heading: #1d4ed8;
      --accent-rule: #3b82f6;
      --sheet-bg: #f8faff;
    }
    /* 13. Metropolitan Boxed (PRO) */
    .style-metro-corporate {
      --accent-heading: #1e293b;
      --accent-rule: #cbd5e1;
      --sheet-bg: #ffffff;
    }
    /* 14. Boutique Creative (PRO) */
    .style-boutique-creative {
      --accent-heading: #292524;
      --accent-rule: #78716c;
      --sheet-bg: #faf8f5;
    }
    /* 15. Apex Monolith (PRO) */
    .style-apex-monolith {
      --accent-heading: #0f172a;
      --accent-rule: #334155;
      --sheet-bg: #ffffff;
    ...
    .cv-sheet-themed-bg { background-color: var(--sheet-bg) !important; }
...
<body class="bg-slate-950 text-slate-100 antialiased min-h-screen flex flex-col justify-between selection:bg-blue-600 selection:text-white style-prestige" id="main-body">
  ...
  <section id="view-editor" class="hidden max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6 flex-grow">
    ...
    <div class="no-print flex flex-col md:flex-row md:items-center justify-between pb-4 mb-5 border-b border-slate-800 gap-4">
      ...
      <!-- KONTROL FITUR & 15 TEMA SELECTOR -->
      <div class="flex flex-wrap items-center gap-2">
        ...
        <!-- 15 TEMA DROPDOWN SELECTOR -->
        <div class="flex items-center bg-slate-900 p-1 rounded-lg border border-slate-800">
          <label class="text-[10px] text-slate-400 font-bold px-2 hidden sm:inline">Tema:</label>
          <select id="theme-selector-dropdown" onchange="handleThemeSelect(this.value)" class="px-2 py-1 text-xs bg-slate-950 border border-slate-800 rounded font-bold text-blue-300 focus:border-blue-500 focus:outline-none cursor-pointer">
            <optgroup label="5 Tema Terbuka (FREE)">
              <option value="prestige">1. Prestige Executive (Free)</option>
              <option value="modern-corporate">2. Modern Corporate (Free)</option>
              <option value="pure-ats">3. Pure Classic ATS (Free)</option>
              <option value="nordic-slate">4. Nordic Slate (Free)</option>
              <option value="silicon-tech">5. Silicon Tech (Free)</option>
            </optgroup>
            <optgroup label="10 Tema Berbayar (GAEKS PRO 🔒)">
              <option value="emerald-horizon">6. Emerald Horizon (PRO 🔒)</option>
              <option value="crimson-elegance">7. Crimson Elegance (PRO 🔒)</option>
              <option value="obsidian-minimal">8. Obsidian Minimal (PRO 🔒)</option>
              <option value="oxford-navy">9. Oxford Two-Tone (PRO 🔒)</option>
              <option value="swiss-editorial">10. Swiss Editorial (PRO 🔒)</option>
              <option value="imperial-gold">11. Imperial Gold (PRO 🔒)</option>
              <option value="azure-sky">12. Azure Blue (PRO 🔒)</option>
              <option value="metro-corporate">13. Metropolitan Boxed (PRO 🔒)</option>
              <option value="boutique-creative">14. Boutique Creative (PRO 🔒)</option>
              <option value="apex-monolith">15. Apex Monolith (PRO 🔒)</option>
            </optgroup>
          </select>
        </div>
      </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
      ...
      <div class="no-print lg:col-span-5 bg-slate-900/95 rounded-2xl border border-slate-800 p-4 sm:p-5 max-h-[85vh] overflow-y-auto relative shadow-xl">
        ...
        <div class="sticky -top-4 -mx-4 -mt-4 p-4 bg-slate-900/95 backdrop-blur-md border-b border-slate-800 z-30 flex space-x-1 overflow-x-auto shadow-sm">
          ...
          <button onclick="switchFormTab('certifications')" id="ftab-btn-certifications" class="px-3 py-1.5 text-xs font-semibold rounded-lg text-slate-400 hover:text-white whitespace-nowrap transition">Sertifikasi & Lisensi</button>
          <button onclick="switchFormTab('extras')" id="ftab-btn-extras" class="px-3 py-1.5 text-xs font-semibold rounded-lg text-slate-400 hover:text-white whitespace-nowrap transition">Informasi Tambahan</button>
        </div>
        ...
        <div class="pt-2">
          ...
          <div id="ftab-content-profile" class="space-y-3.5">
            ...
            <!-- TARGET ROLES (520+ PROFESI DENGAN LIVE SEARCH & DRAG/DROP) -->
            <div class="p-3 bg-slate-950 rounded-xl border border-slate-800 space-y-2.5">
              ...
              <p id="lbl-f-target-roles-desc" class="text-[10px] text-slate-400 leading-tight">
                Riset HRD merekomendasikan <strong>maksimal 3–4 posisi terkait</strong> agar tetap fokus. Gunakan kotak pencarian di bawah untuk memilih dari <strong>520+ profesi</strong> (klik atau drag ke kotak target).
              </p>
            ...
            <div>
              <div class="flex justify-between items-center mb-1">
                <label id="lbl-f-summary" class="text-[11px] text-slate-400">Profil Profesional (Professional Summary)</label>
          ...
          <div id="ftab-content-experience" class="hidden space-y-3">
            <div class="flex justify-between items-center pb-2 border-b border-slate-800">
              <span id="lbl-f-exp-title" class="text-xs font-bold uppercase text-blue-400">Pengalaman Profesional</span>
          ...
          <!-- FORM 3: PENDIDIKAN TERSTRUKTUR STANDAR EKSEKUTIF -->
          <div id="ftab-content-education" class="hidden space-y-3">
            <div class="flex justify-between items-center pb-2 border-b border-slate-800">
              <div>
                ...
                <p class="text-[10px] text-slate-400">Data institusi, gelar, predikat kelulusan (IPK/Honors), serta fokus riset terstruktur.</p>
              </div>
          ...
          <!-- FORM 4: KEAHLIAN & KOMPETENSI -->
          <div id="ftab-content-skills" class="hidden space-y-4">
            <div class="flex justify-between items-center pb-2 border-b border-slate-800">
              <div>
                <span id="lbl-f-skill-group-title" class="text-xs font-bold uppercase text-blue-400">Keahlian & Kompetensi</span>
                <p id="lbl-f-skill-group-desc" class="text-[11px] text-slate-400">Kelompokkan kompetensi inti Anda secara modular untuk memaksimalkan pemindaian ATS.</p>
              </div>
          ...
          <!-- FORM 5: SERTIFIKASI & LISENSI RESMI (MODUL DINAMIS) -->
          <div id="ftab-content-certifications" class="hidden space-y-3.5">
            <div class="flex justify-between items-center pb-2 border-b border-slate-800">
              <div>
                <span id="lbl-f-cert-title" class="text-xs font-bold uppercase text-blue-400">Sertifikasi & Lisensi Resmi</span>
                <p class="text-[10px] text-slate-400">Sertifikasi profesional, lisensi resmi industri, atau izin profesi terakreditasi.</p>
              </div>
              <button onclick="addCertificationItem()" class="text-xs px-2.5 py-1 bg-blue-950 text-blue-300 hover:bg-blue-900 rounded-lg border border-blue-800 transition">
                <span id="lbl-btn-add-cert">+ Tambah Sertifikasi</span>
              </button>
            </div>
            <div id="certifications-list" class="space-y-3"></div>
          </div>

          <!-- FORM 6: INFORMASI TAMBAHAN -->
          <div id="ftab-content-extras" class="hidden space-y-3.5">
            <div class="flex justify-between items-center pb-2 border-b border-slate-800">
              <div>
                <span id="lbl-f-extra-title" class="text-xs font-bold uppercase text-blue-400">Informasi Tambahan</span>
                <p class="text-[10px] text-slate-400">Tambahkan kemampuan bahasa, publikasi ilmiah, asosiasi profesi, atau penghargaan.</p>
              </div>
              <button onclick="addExtraItem()" class="text-xs px-2.5 py-1 bg-blue-950 text-blue-300 hover:bg-blue-900 rounded-lg border border-blue-800 transition">
                <span id="lbl-btn-add-extra">+ Tambah Informasi Tambahan</span>
              </button>
            </div>
            <div id="extras-list" class="space-y-3"></div>
          </div>
        </div>
      ...
      <div class="lg:col-span-7 flex flex-col items-center">
        ...
        <!-- LEMBAR A4 RESUME -->
        <div id="cv-preview-sheet" class="font-cv-body cv-sheet-themed-bg w-full max-w-[780px] text-slate-800 shadow-2xl rounded-sm p-8 sm:p-11 min-h-[1050px] border border-slate-200">
  ...
  <!-- ================= MODAL GAEKS PRO (UPGRADE MEMBER BERBAYAR) ================= -->
  <div id="pro-theme-modal" class="no-print hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
    <div class="bg-slate-900 border border-slate-700 rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4">
      <div class="flex items-start justify-between">
        <div class="flex items-center space-x-2">
          <span class="text-2xl">🔒</span>
          <div>
            <h3 class="font-extrabold text-lg text-white" id="modal-pro-title">Tema Premium (GAEKS PRO)</h3>
            <p class="text-xs text-slate-400">Eksklusif untuk member terdaftar GAEKS Digital.</p>
          </div>
        </div>
        <button onclick="closeProModal()" class="text-slate-400 hover:text-white text-base font-bold">&times;</button>
      </div>

      <div class="p-4 bg-slate-950/80 rounded-xl border border-slate-800 text-xs space-y-2 text-slate-300">
        <div class="flex items-center gap-2 text-amber-400 font-bold">
          <span>✨ Keunggulan Tema Visual PRO:</span>
        </div>
        <ul class="space-y-1.5 list-disc list-inside text-slate-400">
          <li>Desain bermain warna lembar kerja (*Ivory, Emerald, Cool Azure, Linen*).</li>
          <li>Kartu identitas dua warna (*Two-Tone Header Card*) standar C-Level.</li>
          <li>Tipografi editorial presisi tinggi dengan format konsultasi eksekutif.</li>
          <li>Akses penuh ke seluruh 15 varian tema CV tanpa watermark.</li>
        </ul>
      </div>

      <div class="space-y-2 pt-1">
        <a id="btn-upgrade-wa" href="https://wa.me/6285608561745?text=Halo%20Admin%20GAEKS%2C%20saya%20tertarik%20upgrade%20ke%20Member%20Berbayar%20untuk%20membuka%2015%20Tema%20CV%20Premium" target="_blank" class="w-full py-2.5 px-4 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs rounded-xl flex items-center justify-center gap-2 transition shadow-lg shadow-emerald-600/20">
          <span>💬 Buka Akses Member via WhatsApp</span>
        </a>
        <button onclick="previewProThemeTemporarily()" class="w-full py-2 px-4 bg-slate-800 hover:bg-slate-700 text-slate-300 font-semibold text-xs rounded-xl transition">
          Coba Pratinjau Sementara di Layar
        </button>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    let currentLang = 'id';
    let pendingProTheme = null;

    // DEFINISI 15 TEMA (5 FREE + 10 PRO LOCKED)
    const themesConfig = [
      { id: 'prestige', name: 'Prestige Executive', free: true },
      { id: 'modern-corporate', name: 'Modern Corporate', free: true },
      { id: 'pure-ats', name: 'Pure Classic ATS', free: true },
      { id: 'nordic-slate', name: 'Nordic Slate', free: true },
      { id: 'silicon-tech', name: 'Silicon Tech', free: true },
      { id: 'emerald-horizon', name: 'Emerald Horizon (PRO)', free: false },
      { id: 'crimson-elegance', name: 'Crimson Elegance (PRO)', free: false },
      { id: 'obsidian-minimal', name: 'Obsidian Minimal (PRO)', free: false },
      { id: 'oxford-navy', name: 'Oxford Two-Tone (PRO)', free: false },
      { id: 'swiss-editorial', name: 'Swiss Editorial (PRO)', free: false },
      { id: 'imperial-gold', name: 'Imperial Gold (PRO)', free: false },
      { id: 'azure-sky', name: 'Azure Blue (PRO)', free: false },
      { id: 'metro-corporate', name: 'Metropolitan Boxed (PRO)', free: false },
      { id: 'boutique-creative', name: 'Boutique Creative (PRO)', free: false },
      { id: 'apex-monolith', name: 'Apex Monolith (PRO)', free: false }
    ];

    // BANK DATA LENGKAP 526+ PROFESI
    ...
    const i18n = {
      id: {
        ...
        tabCertifications: "Sertifikasi & Lisensi",
        tabExtras: "Informasi Tambahan",
        ...
        lblSummary: "Profil Profesional (Summary)",
        lblExpTitle: "Pengalaman Profesional",
        btnAddExp: "+ Tambah Pengalaman",
        lblEduTitle: "Pendidikan Formal",
        btnAddEdu: "+ Tambah Gelar",
        lblSkillGroupTitle: "Keahlian & Kompetensi",
        ...
        lblCertTitle: "Sertifikasi & Lisensi Resmi",
        btnAddCert: "+ Tambah Sertifikasi",
        lblExtraTitle: "Informasi Tambahan",
        btnAddExtra: "+ Tambah Informasi Tambahan",
        ...
        sheetExtras: "INFORMASI TAMBAHAN",
        majorAccomplishmentLabel: "Pencapaian Kunci:",
        gpaLabel: "IPK & Predikat:",
        thesisLabel: "Fokus Riset & Prestasi Akademik:",
        ...
        emptyCertifications: "Belum ada sertifikasi yang ditambahkan.",
        emptyExtras: "Belum ada informasi tambahan yang ditambahkan."
      },
      en: {
        navHome: "Home",
        ...
        tabCertifications: "Certifications & Licenses",
        tabExtras: "Additional Information",
        ...
        lblExpTitle: "Professional Experience",
        btnAddExp: "+ Add Position",
        lblEduTitle: "Academic Education",
        btnAddEdu: "+ Add Degree",
        lblSkillGroupTitle: "Core Competencies",
        ...
        lblCertTitle: "Certifications & Licenses",
        btnAddCert: "+ Add Certification",
        lblExtraTitle: "Additional Information",
        btnAddExtra: "+ Add Additional Section",
        ...
        sheetExtras: "ADDITIONAL INFORMATION",
        majorAccomplishmentLabel: "Key Impact:",
        gpaLabel: "GPA & Honors:",
        thesisLabel: "Research Highlights & Thesis:",
        ...
        emptyCertifications: "No certifications added yet.",
        emptyExtras: "No additional information added yet."
      }
    };
    ...
    const STORAGE_KEY = 'gaeks_cv_master_list_v17';
    ...
    function getRoleSeparatorForTheme() {
      if (activeLayout === 'prestige' || activeLayout === 'boutique-creative') {
        return `<span class="text-slate-400 font-normal mx-2 select-none">•</span>`;
      } else if (activeLayout === 'modern-corporate' || activeLayout === 'azure-sky' || activeLayout === 'oxford-navy') {
        return `<span class="text-slate-400 font-normal mx-2 select-none">/</span>`;
      } else if (activeLayout === 'pure-ats' || activeLayout === 'obsidian-minimal' || activeLayout === 'swiss-editorial') {
        return `<span class="text-slate-400 font-normal mx-2 select-none">|</span>`;
      } else {
        return `<span class="text-slate-400 font-normal mx-2 select-none">—</span>`;
      }
    ...
    // 100% BLANK CV DOCUMENT SAAT MEMULAI
    ...
    // LOAD CV LIST (BEBAS DARI SAMPLE DENY TRIAWAN)
    function loadCvList() {
      ...
      if (!cvList || cvList.length === 0) {
        cvList = [createBlankCvObject(currentLang === 'en' ? 'My First Resume' : 'CV Pertama Saya')];
        saveCvList();
      }
      autoPurgeTrash();
    }
          ...
          <div class="p-5 bg-slate-900/90 rounded-2xl border border-slate-800 hover:border-slate-700 transition flex flex-col justify-between shadow-lg">
            <div>
              <div class="flex items-start justify-between gap-2 mb-2">
                ...
                <span class="text-[10px] uppercase font-semibold px-2 py-0.5 rounded bg-blue-950 text-blue-300 border border-blue-800">Executive</span>
              </div>
    ...
    function loadFormData(d) {
      ...
      renderCertificationsList(d.certificationsList || []);
      renderExtrasList(d.extrasList || []);
      
      const targetLyt = d.layout || 'prestige';
      document.getElementById('theme-selector-dropdown').value = targetLyt;
      setThemeLayout(targetLyt);
    }
    ...
    function extractFormData() {
      ...
      document.querySelectorAll('.edu-item').forEach(el => {
        edus.push({
          ...
          location: el.querySelector('.edu-loc') ? el.querySelector('.edu-loc').value : '',
          degree: el.querySelector('.edu-deg').value,
          period: el.querySelector('.edu-period').value,
          gpa: el.querySelector('.edu-gpa') ? el.querySelector('.edu-gpa').value : '',
      ...
      document.querySelectorAll('.cert-item').forEach(el => {
        certs.push({
          ...
          period: el.querySelector('.cert-period').value,
      ...
      document.querySelectorAll('.extra-item').forEach(el => {
        extras.push({
          type: el.querySelector('.extra-type').value,
          details: el.querySelector('.extra-desc').value
        });
      });
    ...
    function addSkillGroup(group = { category: 'Keahlian Teknis & Inti', skills: '' }) {
      ...
      div.innerHTML = `
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
          <div>
            <label class="block text-[10px] text-slate-400 mb-0.5">${currentLang === 'en' ? 'Category Template' : 'Pilih Kategori Keahlian'}</label>
            <select onchange="handleSkillCategoryDropdown(this)" class="w-full px-2.5 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded text-white focus:border-blue-500 focus:outline-none">
              ...
              <option value="custom">-- ${currentLang === 'en' ? 'Custom Category' : 'Kategori Kustom'} --</option>
            </select>
          </div>
          <div>
    ...
    function handleSkillCategoryDropdown(select) {
      ...
      if (select.value === 'custom') {
        customInput.value = '';
        customInput.placeholder = currentLang === 'en' ? 'Type custom category...' : 'Ketik kategori...';
        customInput.focus();
      } else {
    ...
    // FORM PENDIDIKAN TERSTRUKTUR
    function addEducation(item = { school: '', location: '', degree: '', period: '', gpa: '', details: '' }) {
      ...
      div.innerHTML = `
        ...
        <div class="grid grid-cols-2 gap-2">
          <div>
            ...
            <input type="text" placeholder="${currentLang === 'en' ? 'Diponegoro University' : 'Nama Kampus'}" value="${item.school || ''}" oninput="handleInputChange()" class="edu-school w-full px-2.5 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded text-white focus:border-blue-500 focus:outline-none" />
          </div>
          <div>
            <label class="block text-[10px] text-slate-400 mb-0.5">${currentLang === 'en' ? 'Location (City, Country)' : 'Lokasi (Kota, Negara)'}</label>
            <input type="text" placeholder="${currentLang === 'en' ? 'Semarang, Indonesia' : 'Kota / Lokasi'}" value="${item.location || ''}" oninput="handleInputChange()" class="edu-loc w-full px-2.5 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded text-white focus:border-blue-500 focus:outline-none" />
          </div>
        </div>
        <div class="grid grid-cols-2 gap-2">
          <div>
            <label class="block text-[10px] text-slate-400 mb-0.5">${currentLang === 'en' ? 'Degree & Major' : 'Gelar & Program Studi / Jurusan'}</label>
            <input type="text" placeholder="${currentLang === 'en' ? 'Master of Information System (M.IS)' : 'Gelar & Jurusan'}" value="${item.degree || ''}" oninput="handleInputChange()" class="edu-deg w-full px-2.5 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded text-white focus:border-blue-500 focus:outline-none" />
          </div>
          <div>
            <label class="block text-[10px] text-slate-400 mb-0.5">${currentLang === 'en' ? 'Graduation Year / Period' : 'Tahun Kelulusan / Periode'}</label>
            <input type="text" placeholder="${currentLang === 'en' ? '2017 - 2020' : 'Tahun Kelulusan'}" value="${item.period || ''}" oninput="handleInputChange()" class="edu-period w-full px-2.5 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded text-white focus:border-blue-500 focus:outline-none" />
          </div>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
          <div>
            <label class="block text-[10px] text-slate-400 mb-0.5">${currentLang === 'en' ? 'GPA & Academic Honors' : 'IPK & Predikat Kelulusan'}</label>
            <input type="text" placeholder="${currentLang === 'en' ? 'GPA 3.84 (Cum Laude)' : 'IPK 3.38 (Sangat Memuaskan)'}" value="${item.gpa || ''}" oninput="handleInputChange()" class="edu-gpa w-full px-2.5 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded text-white focus:border-blue-500 focus:outline-none" />
          </div>
          <div>
            <label class="block text-[10px] text-slate-400 mb-0.5">${currentLang === 'en' ? 'Research Highlights / Thesis' : 'Fokus Riset / Tesis / Prestasi Akademik'}</label>
            <input type="text" placeholder="${currentLang === 'en' ? 'Best Research Paper Award IEEE SIET' : 'Fokus Riset / Skripsi / Tesis'}" value="${item.details || ''}" oninput="handleInputChange()" class="edu-det w-full px-2.5 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded text-white focus:border-blue-500 focus:outline-none" />
          </div>
        </div>
      `;
      list.appendChild(div);
    ...
    // FORM SERTIFIKASI & LISENSI
    function addCertificationItem(item = { name: '', issuer: '', period: '', credentialId: '' }) {
      const list = document.getElementById('certifications-list');
      const div = document.createElement('div');
      div.className = "cert-item p-3 bg-slate-950 rounded-xl border border-slate-800 relative space-y-2 transition-all";
      div.innerHTML = `
        <button onclick="this.parentElement.remove(); handleInputChange();" class="absolute top-2.5 right-2.5 text-xs text-rose-400 hover:text-rose-300">✕ ${currentLang === 'en' ? 'Delete' : 'Hapus'}</button>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
          <div>
            <label class="block text-[10px] text-slate-400 mb-0.5">${currentLang === 'en' ? 'Certification / License Name' : 'Nama Sertifikasi / Lisensi'}</label>
            <input type="text" placeholder="Database Design and Programming with SQL" value="${item.name || ''}" oninput="handleInputChange()" class="cert-name w-full px-2.5 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded text-white focus:border-blue-500 focus:outline-none" />
          </div>
          <div>
            <label class="block text-[10px] text-slate-400 mb-0.5">${currentLang === 'en' ? 'Issuing Authority / Institution' : 'Lembaga Penerbit / Otoritas'}</label>
            <input type="text" placeholder="Oracle Academy / BNSP" value="${item.issuer || ''}" oninput="handleInputChange()" class="cert-issuer w-full px-2.5 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded text-white focus:border-blue-500 focus:outline-none" />
          </div>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
          <div>
            <label class="block text-[10px] text-slate-400 mb-0.5">${currentLang === 'en' ? 'Issue Date / Validity' : 'Tahun Terbit / Masa Berlaku'}</label>
            <input type="text" placeholder="2022 - 2025 atau Seumur Hidup" value="${item.period || ''}" oninput="handleInputChange()" class="cert-period w-full px-2.5 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded text-white focus:border-blue-500 focus:outline-none" />
          </div>
          <div>
            <label class="block text-[10px] text-slate-400 mb-0.5">${currentLang === 'en' ? 'Credential ID / Details (Optional)' : 'Nomor Lisensi / Kredensial (Opsional)'}</label>
            <input type="text" placeholder="ID: ORCL-78901 / Nilai: Mastery" value="${item.credentialId || ''}" oninput="handleInputChange()" class="cert-id w-full px-2.5 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded text-white focus:border-blue-500 focus:outline-none" />
          </div>
        </div>
      `;
      list.appendChild(div);
    ...
    function addExtraItem(item = { type: 'Bahasa', details: '' }) {
      const list = document.getElementById('extras-list');
      const div = document.createElement('div');
      div.className = "extra-item p-3 bg-slate-950 rounded-xl border border-slate-800 relative space-y-2 transition-all";
      div.innerHTML = `
        <button onclick="this.parentElement.remove(); handleInputChange();" class="absolute top-2.5 right-2.5 text-xs text-rose-400 hover:text-rose-300">✕ ${currentLang === 'en' ? 'Delete' : 'Hapus'}</button>
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
          <div>
            <label class="block text-[10px] text-slate-400 mb-0.5">${currentLang === 'en' ? 'Category / Type' : 'Kategori / Bagian'}</label>
            <input type="text" placeholder="mis: Bahasa / Publikasi" value="${item.type || 'Bahasa'}" oninput="handleInputChange()" class="extra-type w-full px-2.5 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded text-white focus:border-blue-500 focus:outline-none font-semibold" />
          </div>
          <div class="sm:col-span-2">
            <label class="block text-[10px] text-slate-400 mb-0.5">${currentLang === 'en' ? 'Description & Details' : 'Deskripsi & Uraian'}</label>
            <input type="text" placeholder="mis: Bahasa Indonesia (Native), English (C1 Proficient)" value="${item.details || ''}" oninput="handleInputChange()" class="extra-desc w-full px-2.5 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded text-white focus:border-blue-500 focus:outline-none" />
          </div>
        </div>
      `;
      list.appendChild(div);
    ...
    function renderCertificationsList(certs) {
      ...
      if (certs && certs.length > 0) {
        certs.forEach(c => addCertificationItem(c));
      } else {
    ...
    function renderExtrasList(extras) {
      ...
      if (extras && extras.length > 0) {
        extras.forEach(e => addExtraItem(e));
      } else {
        addExtraItem({ type: 'Bahasa', details: 'Bahasa Indonesia (Native), English (C1 Proficient)' });
      }
    }
    ...
    function switchFormTab(tab) {
      ['profile', 'experience', 'education', 'skills', 'certifications', 'extras'].forEach(t => {
    ...
    // ================= 15 TEMA HANDLER (FREE VS PRO LOCKED) =================
    function handleThemeSelect(themeId) {
      const target = themesConfig.find(t => t.id === themeId);
      if (!target) return;

      if (!target.free) {
        pendingProTheme = target;
        showProModal(target);
        return;
      }

      setThemeLayout(themeId);
    }

    function showProModal(theme) {
      document.getElementById('modal-pro-title').innerText = `Tema ${theme.name} (GAEKS PRO 🔒)`;
      const waLink = `https://wa.me/6285608561745?text=Halo%20Admin%20GAEKS%2C%20saya%20tertarik%20upgrade%20ke%20Member%20Berbayar%20untuk%20membuka%2015%20Tema%20CV%20Premium%20(Tema%3A%20${encodeURIComponent(theme.name)})`;
      document.getElementById('btn-upgrade-wa').href = waLink;
      document.getElementById('pro-theme-modal').classList.remove('hidden');
    }

    function closeProModal() {
      document.getElementById('pro-theme-modal').classList.add('hidden');
      document.getElementById('theme-selector-dropdown').value = activeLayout;
      pendingProTheme = null;
    }

    function previewProThemeTemporarily() {
      if (pendingProTheme) {
        setThemeLayout(pendingProTheme.id);
        document.getElementById('theme-selector-dropdown').value = pendingProTheme.id;
      }
      closeProModal();
    }
    ...
    function setThemeLayout(name) {
      ...
      const current = themesConfig.find(t => t.id === name);
      document.getElementById('label-active-layout').innerText = current ? current.name : name;

      const cv = cvList.find(c => c.id === currentCvId);
      if (cv) {
        cv.data.layout = activeLayout;
        cv.updatedAt = Date.now();
        saveCvList();
      }
    ...
    function setLanguage(lang) {
      ...
      document.getElementById('ftab-btn-certifications').innerText = dict.tabCertifications;
      ...
      document.getElementById('lbl-f-cert-title').innerText = dict.lblCertTitle;
      document.getElementById('lbl-btn-add-cert').innerText = dict.btnAddCert;
      document.getElementById('lbl-f-extra-title').innerText = dict.lblExtraTitle;
      document.getElementById('lbl-btn-add-extra').innerText = dict.btnAddExtra;
    ...
    function renderCV() {
      ...
      // GENERATOR JUDUL KATEGORI RESMI BERDASARKAN 15 TEMA
      function renderCategoryHeader(titleText) {
        if (activeLayout === 'prestige' || activeLayout === 'boutique-creative') {
          return `
            <div class="cv-section-title border-b border-slate-900 pb-0.5 mb-2 mt-3 flex items-center justify-between">
              <h2 class="text-[12px] font-bold font-prestige uppercase tracking-wider heading-accent">${titleText}</h2>
          ...
          return `
            <div class="cv-section-title border-b border-black pb-0.5 mb-2 mt-3 text-center">
              <h2 class="text-[11.5px] font-bold uppercase tracking-widest text-black">${titleText}</h2>
            </div>
          `;
        } else if (activeLayout === 'metro-corporate') {
          return `
            <div class="cv-section-title mb-2 mt-3 flex items-center">
              <span class="px-2.5 py-0.5 bg-slate-100 border border-slate-300 rounded text-[11px] font-extrabold uppercase tracking-wider heading-accent">${titleText}</span>
            </div>
          `;
        } else if (activeLayout === 'apex-monolith') {
          return `
            <div class="cv-section-title border-y-2 border-slate-800 py-0.5 mb-2 mt-3 flex items-center justify-between">
              <h2 class="text-[11.5px] font-black uppercase tracking-wider heading-accent">${titleText}</h2>
              <span class="text-[9px] font-mono text-slate-500 uppercase tracking-widest">STANDARD ATS</span>
            </div>
          `;
        } else if (activeLayout === 'swiss-editorial') {
          return `
            <div class="cv-section-title border-l-4 rule-border pl-2 pb-0.5 mb-2 mt-3">
              <h2 class="text-[11.5px] font-extrabold uppercase tracking-wider heading-accent">${titleText}</h2>
            </div>
          `;
        } else {
          return `
            <div class="cv-section-title border-b-2 rule-border pb-0.5 mb-2 mt-3 flex items-center gap-2">
              <span class="w-1.5 h-3 rule-border border-l-2"></span>
              <h2 class="text-[11.5px] font-bold uppercase tracking-wider heading-accent">${titleText}</h2>
            </div>
          `;
        }
      }

      // EXPERIENCES: PEMADATAN MARGIN & HAIRLINE SEPARATOR HALUS
      let experiencesHTML = '';
      ...
      if (expElements.length === 0) {
        ...
        expElements.forEach((el) => {
          ...
          const accParagraph = acc.trim() ? `
            <div class="mt-1 text-[11px] text-slate-800 leading-normal pl-0.5">
          ...
          experiencesHTML += `
            <div class="cv-entry-block mb-2 pb-1.5 border-b border-slate-200/50 last:border-b-0 last:pb-0 last:mb-0.5">
              ...
              <div class="flex justify-between items-baseline text-[11px] text-slate-600 mb-0.5">
      ...
      if (eduElements.length === 0) {
        ...
        eduElements.forEach(el => {
          ...
          const eduLoc = applyTranslation(el.querySelector('.edu-loc') ? el.querySelector('.edu-loc').value : '');
          const deg = applyTranslation(el.querySelector('.edu-deg').value);
          const per = applyTranslation(el.querySelector('.edu-period').value);
          const gpaVal = el.querySelector('.edu-gpa') ? el.querySelector('.edu-gpa').value : '';
          ...
          if (gpaVal.trim()) {
            gpaRow = `
              <div class="text-[11px] text-slate-700 mt-0.5">
                <span class="font-semibold text-slate-900">${lbl.gpaLabel}</span> 
                <span>${gpaVal}</span>
              </div>
            `;
          }
          ...
          if (det.trim()) {
            detailsRow = `
              <div class="text-[11px] text-slate-600 mt-0.5 leading-relaxed text-justify">
                <span class="font-semibold text-slate-900">${lbl.thesisLabel}</span> 
                <span>${det}</span>
              </div>
            `;
          }
          ...
          educationsHTML += `
            <div class="cv-entry-block mb-2 pb-1.5 border-b border-slate-200/40 last:border-b-0 last:pb-0 last:mb-0.5 text-xs">
              <div class="flex justify-between items-baseline">
                <span class="font-bold text-slate-950 text-[12.5px]">${school || 'Institusi / Universitas'}</span>
                <span class="text-slate-500 text-[11px] italic shrink-0">${eduLoc}</span>
              </div>
              <div class="flex justify-between items-baseline text-[11.5px] text-slate-800 mt-0.5">
                <span class="font-medium ${activeLayout === 'prestige' ? 'font-prestige' : ''}">${deg || 'Gelar & Program Studi'}</span>
                <span class="font-semibold text-slate-700 text-[11px] shrink-0">${per}</span>
              </div>
              ${gpaRow}
              ${detailsRow}
      ...
      document.querySelectorAll('.skill-group-item').forEach(el => {
        ...
        if (sks.trim()) {
          skillsGroupHTML += `
            <div class="text-[11px] leading-relaxed mb-0.5 flex flex-col sm:flex-row sm:items-baseline gap-1">
              <span class="font-semibold text-slate-900 shrink-0 sm:min-w-[150px]">${cat}:</span> 
      ...
      if (certElements.length === 0) {
        certsHTML = `<p class="text-[11px] text-slate-400 italic">${lbl.emptyCertifications}</p>`;
      } else {
        certElements.forEach(el => {
          const cName = applyTranslation(el.querySelector('.cert-name').value);
          const cIssuer = applyTranslation(el.querySelector('.cert-issuer').value);
          const cPeriod = applyTranslation(el.querySelector('.cert-period').value);
          const cId = el.querySelector('.cert-id') ? el.querySelector('.cert-id').value : '';

          if (cName.trim() || cIssuer.trim()) {
            certsHTML += `
              <div class="cv-entry-block mb-1.5 pb-1 border-b border-slate-200/40 last:border-b-0 last:pb-0 last:mb-0.5 text-xs">
                <div class="flex justify-between items-baseline">
                  <span class="font-semibold text-slate-950 text-[11.5px]">${cName || 'Nama Sertifikasi / Lisensi'}</span>
                  <span class="text-slate-600 text-[10.5px] shrink-0">${cPeriod}</span>
                </div>
                <div class="flex justify-between items-baseline text-[11px] text-slate-600">
                  <span class="italic text-slate-700">${cIssuer}</span>
                  ${cId ? `<span class="text-slate-500 font-mono text-[10px]">${cId}</span>` : ''}
                </div>
              </div>
            `;
          }
      ...
      let extrasHTML = '';
      const extraElements = document.querySelectorAll('.extra-item');
      if (extraElements.length === 0) {
        extrasHTML = `<p class="text-[11px] text-slate-400 italic">${lbl.emptyExtras}</p>`;
      } else {
        extraElements.forEach(el => {
          const eType = applyTranslation(el.querySelector('.extra-type').value);
          const eDesc = applyTranslation(el.querySelector('.extra-desc').value);
          if (eType.trim() && eDesc.trim()) {
            extrasHTML += `
              <div class="text-[11px] leading-relaxed mb-1 flex flex-col sm:flex-row sm:items-baseline gap-1">
                <span class="font-semibold text-slate-900 shrink-0 sm:min-w-[150px]">${eType}:</span>
                <span class="text-slate-700">${eDesc}</span>
              </div>
            `;
          }
        });
      ...
      // ================= HEADER DUA WARNA ATAU HEADER SIMETRIS =================
      let headerHTML = '';
      const isOxfordNavy = (activeLayout === 'oxford-navy');

      if (hasPhoto) {
        headerHTML = `
          <div class="pb-3 border-b-2 rule-border flex items-center gap-5 ${isOxfordNavy ? 'p-4 bg-slate-900 text-white rounded-t-lg -mx-8 -mt-8 sm:-mx-11 sm:-mt-11 mb-3' : (activeLayout === 'modern-corporate' ? 'p-3.5 bg-slate-50 rounded-lg' : '')}">
            <div class="shrink-0">
            ...
            <div class="flex-grow flex flex-col justify-center min-h-[112px]">
              <h1 class="text-2xl sm:text-[26px] font-bold tracking-tight ${isOxfordNavy ? 'text-white' : 'text-slate-950'} ${activeLayout === 'prestige' ? 'font-prestige' : ''} leading-none">${name}</h1>
              <p class="text-[11.5px] sm:text-[12px] font-semibold ${isOxfordNavy ? 'text-blue-200' : 'text-slate-700'} mt-1.5 tracking-wide leading-snug">${rolesString}</p>
              ${contactLine}
            </div>
          </div>
        `;
        ...
        headerHTML = `
          <div class="pb-3 border-b-2 rule-border ${isOxfordNavy ? 'p-4 bg-slate-900 text-white rounded-t-lg -mx-8 -mt-8 sm:-mx-11 sm:-mt-11 mb-3' : (activeLayout === 'pure-ats' ? 'text-center' : (activeLayout === 'modern-corporate' ? 'p-3.5 bg-slate-50 rounded-lg' : 'text-center sm:text-left'))}">
            <h1 class="text-2xl sm:text-[26px] font-bold tracking-tight ${isOxfordNavy ? 'text-white' : 'text-slate-950'} ${activeLayout === 'prestige' ? 'font-prestige' : ''} leading-none">${name}</h1>
            <p class="text-[11.5px] sm:text-[12px] font-semibold ${isOxfordNavy ? 'text-blue-200' : 'text-slate-700'} mt-1.5 tracking-wide leading-snug">${rolesString}</p>
            ${contactLine}
          </div>
      ...
      root.innerHTML = `
        ...
        ${extrasHTML ? `
          <div class="mt-2.5">
            ${renderCategoryHeader(lbl.sheetExtras)}
            <div class="space-y-0.5">${extrasHTML}</div>
          </div>
        ` : ''}
      `;
    }
...
echo "=== 2. Mem-push pembaruan ke GitHub ==="
git add cv.html
git commit -m "feat: clean sample data and implement 15 visual themes with 5 free and 10 PRO locked variants" || true
