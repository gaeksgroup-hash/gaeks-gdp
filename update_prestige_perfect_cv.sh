...
echo "=== 1. Memperbarui cv.html dengan Header Simetris, Prestige Theme & Modern ATS Engine ==="
...
<body class="bg-slate-950 text-slate-100 antialiased min-h-screen flex flex-col justify-between selection:bg-blue-600 selection:text-white style-prestige" id="main-body">
  ...
  <script>
    ...
    let historyStack = [];
    let historyIndex = -1;
    const STORAGE_KEY = 'gaeks_cv_master_list_v10';
    ...
    function renderCV() {
      ...
      const contactLine = contactItems.length > 0 
        ? `<div class="flex flex-wrap items-center justify-center sm:justify-start gap-y-1 text-[11px] text-slate-600 mt-1 font-normal">${contactItems.join(separatorHTML)}</div>`
        : '';

      // GENERATOR JUDUL KATEGORI
      ...
      if (hasPhoto) {
        headerHTML = `
          <div class="pb-3 border-b-2 rule-border flex items-center gap-5 ${activeLayout === 'modern-corporate' ? 'p-3.5 bg-slate-50 rounded-lg' : ''}">
            <div class="shrink-0">
            ...
            <div class="flex-grow flex flex-col justify-center min-h-[112px]">
              ...
              <p class="text-[12px] sm:text-[12.5px] font-semibold text-slate-700 mt-1.5 tracking-wide leading-snug">${title}</p>
              ${contactLine}
            </div>
          </div>
        `;
        ...
        headerHTML = `
          <div class="pb-3 border-b-2 rule-border ${activeLayout === 'pure-ats' ? 'text-center' : 'text-center sm:text-left'} ${activeLayout === 'modern-corporate' ? 'p-3.5 bg-slate-50 rounded-lg' : ''}">
            <h1 class="text-2xl sm:text-[26px] font-bold tracking-tight text-slate-950 ${activeLayout === 'prestige' ? 'font-prestige' : ''} leading-none">${name}</h1>
            <p class="text-[12px] sm:text-[12.5px] font-semibold text-slate-700 mt-1.5 tracking-wide leading-snug">${title}</p>
            ${contactLine}
          </div>
...
echo "=== 2. Mem-push pembaruan Prestige Symmetrical & Anti-Cut PDF ke GitHub ==="
git add cv.html
git commit -m "feat: perfect symmetrical photo-header alignment with Prestige executive styling and anti-cut PDF rules" || true
