...
echo "=== 1. Memperbarui cv.html dengan Drag & Drop Target Roles, Separator Tema & Clean Executive Styling ==="
...
<body class="bg-slate-950 text-slate-100 antialiased min-h-screen flex flex-col justify-between selection:bg-blue-600 selection:text-white style-prestige" id="main-body">
      ...
      <div class="no-print lg:col-span-5 bg-slate-900/95 rounded-2xl border border-slate-800 p-4 sm:p-5 max-h-[85vh] overflow-y-auto relative shadow-xl">
        ...
        <div class="pt-2">
          ...
          <div id="ftab-content-profile" class="space-y-3.5">
            ...
            <!-- MULTIPLE CHOICE TARGET ROLES (DRAG & DROP + KLIK + INPUT MANUAL) -->
            <div class="p-3 bg-slate-950 rounded-xl border border-slate-800 space-y-2.5">
              ...
              <p id="lbl-f-target-roles-desc" class="text-[10px] text-slate-400 leading-tight">
                Riset HRD merekomendasikan <strong>maksimal 3–4 posisi terkait</strong> agar tetap fokus. Anda dapat <strong>klik</strong> atau <strong>seret (drag & drop)</strong> profesi dari bank data ke kotak di bawah.
              </p>
              ...
              <!-- BANK DATA PROFESI (DRAGGABLE) -->
              <div class="pt-1">
                <div class="flex items-center justify-between mb-1">
                  <span class="text-[10px] text-slate-400 font-medium">Bank Saran Profesi (Klik atau Seret):</span>
                  <select id="role-category-filter" onchange="renderRoleSuggestions()" class="text-[10px] bg-slate-900 border border-slate-800 rounded px-2 py-0.5 text-slate-300 focus:outline-none">
                    <option value="logistics">Logistik & Supply Chain</option>
                    <option value="tech">IT, Software & Data</option>
                    <option value="management">Bisnis & Eksekutif</option>
                    <option value="finance">Keuangan & Akuntansi</option>
                    <option value="marketing">Pemasaran & Sales</option>
                    <option value="engineering">Teknik & Manufaktur</option>
                    <option value="hr">SDM, Talent & Legal</option>
                  </select>
                </div>
                <div id="role-suggestion-chips" class="flex flex-wrap gap-1 max-h-24 overflow-y-auto p-1 bg-slate-900/60 rounded border border-slate-800/80"></div>
              </div>
            </div>
  ...
  <script>
    ...
    // BANK DATA PROFESI LENGKAP
    const professionBank = {
      logistics: [
        "Supply Chain Manager", "Logistics Operations Manager", "Export-Import Specialist",
        "Freight Forwarding Manager", "Customs Clearance Specialist (PPJK)", "Warehouse Operations Manager",
        "Procurement & Sourcing Lead", "Multimodal Transport Coordinator"
      ],
      tech: [
        "ERP Solution Architect", "Odoo ERP Implementer", "Full-Stack Engineer",
        "DevOps & Cloud Engineer", "Data Engineer / Analytics Lead", "Product Manager",
        "Systems Analyst", "Cybersecurity Specialist"
      ],
      management: [
        "Head of Operations", "Chief Operating Officer (COO)", "General Manager",
        "Project Management Professional (PMP)", "Operational Excellence Lead",
        "Business Development Manager", "Management Consultant"
      ],
      finance: [
        "Financial Controller", "Corporate Auditor", "Finance & Accounting Manager",
        "Tax Specialist (Brevet AB)", "Financial Analyst", "Budgeting Specialist"
      ],
      marketing: [
        "Digital Marketing Lead", "Brand Strategist", "B2B Sales Director",
        "Growth Marketing Specialist", "Key Account Manager"
      ],
      engineering: [
        "Industrial Engineer", "Quality Assurance Lead (QA/QC)", "Lean Six Sigma Black Belt",
        "Plant Manager", "Automation Engineer"
      ],
      hr: [
        "HR Business Partner (HRBP)", "Talent Acquisition Lead",
        "People Operations Manager", "Corporate Legal Counsel"
      ]
    };
    ...
    const i18n = {
      id: {
        ...
        lblTargetRolesDesc: "Riset HRD merekomendasikan maksimal 3–4 posisi terkait agar tetap fokus. Anda dapat klik atau seret (drag & drop) profesi ke kotak target.",
        lblEmail: "Email",
      ...
      en: {
        ...
        lblTargetRolesDesc: "HR research recommends 3–4 related titles maximum to maintain clear focus. Click or drag & drop roles into the target box.",
    ...
    const STORAGE_KEY = 'gaeks_cv_master_list_v12';

    // DRAG & DROP SERTA BANK SARAN PROFESI
    ...
    // PEMISAH TARGET POSISI BERBEDA DI SETIAP TEMA
    ...
    function addExperience(item = { role: '', company: '', period: '', location: '', desc: '', accomplishment: '' }) {
      ...
      div.innerHTML = `
        ...
        <div>
          <label class="block text-[10px] text-amber-400 font-semibold mb-0.5">${currentLang === 'en' ? 'Major Accomplishment / Key Impact (Rendered as Bold Statement)' : 'Pencapaian Utama (Diformat Sebagai Pernyataan Tebal Eksekutif)'}</label>
    ...
    function renderCV() {
      ...
      if (currentTargetRoles && currentTargetRoles.length > 0) {
        ...
        rolesString = currentLang === 'en' ? 'Target Executive Roles / Professional Title' : 'Target Posisi / Profesi';
      }
      ...
      // EXPERIENCES: DUA BARIS SEJAJAR + MAJOR ACCOMPLISHMENT PARAGRAF TEBAL
      let experiencesHTML = '';
      ...
      if (expElements.length === 0) {
        ...
        expElements.forEach(el => {
          ...
          // MAJOR ACCOMPLISHMENT SBG PARAGRAF NARASI TEBAL (BEBAS BULLETS/DOT & BOX AI)
          ...
          experiencesHTML += `
            <div class="cv-entry-block mb-3 last:mb-1">
              <div class="flex justify-between items-baseline text-xs">
                <span class="font-bold text-slate-950 text-[12.5px] tracking-tight">${role || (currentLang === 'en' ? 'Executive Role' : 'Jabatan')}</span>
...
git add cv.html
git commit -m "feat: implement drag-and-drop target roles with theme-based separators and bold narrative accomplishment" || true
