...
echo "=== Memperbarui cv.html: Dashboard List, Trash 3 Hari, Undo/Redo & Advanced Translator ==="
...
<head>
  ...
  <title>GAEKS ATS CV Studio | Dashboard & Resume Manager</title>
...
<body class="bg-slate-950 text-slate-100 antialiased min-h-screen flex flex-col justify-between selection:bg-blue-600 selection:text-white">
  ...
  <section id="view-dashboard" class="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 flex-grow">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between pb-6 mb-6 border-b border-slate-800 gap-4">
      <div>
        ...
        <p class="text-xs sm:text-sm text-slate-400 mt-1">Kelola portofolio CV Anda, duplikasi untuk variasi lamaran, atau buat resume baru.</p>
      </div>
      <button onclick="createNewCV()" class="px-4 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-bold text-xs sm:text-sm shadow-lg shadow-blue-500/20 flex items-center space-x-2 transition self-start sm:self-auto">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
        <span>+ Buat CV Baru</span>
      </button>
    </div>

    <!-- TABS: SEMUA CV vs TEMPAT SAMPAH (TRASH) -->
    ...
    <!-- NOTIFIKASI TRASH RETENSI 3 HARI -->
    <div id="trash-notice" class="hidden mb-6 p-3.5 bg-amber-950/40 border border-amber-800/60 rounded-xl text-xs text-amber-300 flex items-center gap-2">
      ...
      <span>Dokumen di tempat sampah tersimpan selama <strong>3 hari</strong> sebelum dihapus secara otomatis dan permanen oleh sistem.</span>
    </div>
  ...
  <section id="view-editor" class="hidden max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6 flex-grow">
    ...
    <!-- SUBBAR TOOLBAR -->
    <div class="no-print flex flex-col md:flex-row md:items-center justify-between pb-5 mb-5 border-b border-slate-800 gap-4">
      <div class="flex items-center space-x-3">
      ...
      <!-- UNDO / REDO & LANGUAGE CONTROLS -->
      <div class="flex flex-wrap items-center gap-2">
        <!-- UNDO & REDO BUTTONS -->
        <div class="flex items-center bg-slate-900 p-1 rounded-lg border border-slate-800 space-x-1">
          <button onclick="undo()" id="btn-undo" title="Undo (Ctrl+Z)" disabled class="p-1.5 rounded text-xs text-slate-500 disabled:opacity-40 hover:text-white transition">
            ↶ Undo
          </button>
          <button onclick="redo()" id="btn-redo" title="Redo (Ctrl+Y)" disabled class="p-1.5 rounded text-xs text-slate-500 disabled:opacity-40 hover:text-white transition">
            ↷ Redo
          </button>
        </div>
        ...
        <div class="flex items-center space-x-1 bg-slate-900 p-1 rounded-lg border border-slate-800 text-xs">
          <button onclick="setTheme('navy')" id="btn-theme-navy" class="px-2 py-1 rounded font-semibold bg-blue-950 text-blue-300 border border-blue-600">Navy</button>
          <button onclick="setTheme('emerald')" id="btn-theme-emerald" class="px-2 py-1 rounded font-semibold text-slate-400 hover:text-white">Emerald</button>
          <button onclick="setTheme('slate')" id="btn-theme-slate" class="px-2 py-1 rounded font-semibold text-slate-400 hover:text-white">Slate</button>
        </div>
      </div>
    </div>
      ...
      <div class="no-print lg:col-span-5 bg-slate-900/90 rounded-2xl border border-slate-800 p-4 sm:p-5 max-h-[85vh] overflow-y-auto space-y-4">
        ...
        <div id="ftab-content-profile" class="space-y-3">
          ...
          <div id="photo-box-input" class="hidden p-3 bg-slate-950 rounded-xl border border-slate-800 flex items-center gap-3">
            ...
            <div class="text-xs flex-grow">
              <span class="block text-slate-300 font-medium mb-1">Upload Foto Formal</span>
          ...
          <div>
            <div class="flex justify-between items-center mb-1">
              ...
              <button onclick="aiTranslateField('input-summary')" type="button" class="text-[10px] text-blue-400 hover:text-blue-300 font-semibold">
                ✨ Format ke English
              </button>
            </div>
        ...
        <div id="ftab-content-experience" class="hidden space-y-3">
          <div class="flex justify-between items-center pb-2 border-b border-slate-800">
            ...
            <button onclick="addExperience()" class="text-xs px-2.5 py-1 bg-blue-950 text-blue-300 hover:bg-blue-900 rounded-lg border border-blue-800">+ Tambah Posisi</button>
          </div>
        ...
        <div id="ftab-content-education" class="hidden space-y-3">
          <div class="flex justify-between items-center pb-2 border-b border-slate-800">
            <span class="text-xs font-bold uppercase text-blue-400">Pendidikan</span>
            <button onclick="addEducation()" class="text-xs px-2.5 py-1 bg-blue-950 text-blue-300 hover:bg-blue-900 rounded-lg border border-blue-800">+ Tambah Gelar</button>
          </div>
        ...
        <div id="ftab-content-skills" class="hidden space-y-3">
          ...
          <div>
            <label class="block text-[11px] text-slate-400 mb-1">Sertifikasi & Lisensi (Tiap baris 1 lisensi)</label>
            <textarea id="input-certifications" rows="2" oninput="handleInputChange()" class="w-full px-3 py-2 text-xs sm:text-sm bg-slate-950 border border-slate-800 rounded-lg text-white"></textarea>
          </div>
        </div>

      </div>

      <!-- PRATINJAU LEMBAR A4 (7 KOLOM) -->
        ...
        <div id="cv-preview-sheet" class="cv-sheet-font w-full max-w-[780px] bg-white text-slate-800 shadow-2xl rounded-sm p-8 sm:p-12 min-h-[1050px] border border-slate-200">
          ...
          <div id="cv-header-container" class="pb-4 border-b theme-accent-border flex flex-col sm:flex-row items-center sm:items-start gap-5">
            ...
            <div class="flex-grow text-center sm:text-left">
              <h1 id="view-name" class="text-2xl sm:text-3xl font-extrabold uppercase tracking-tight theme-accent-text">NAMA LENGKAP</h1>
              <p id="view-title" class="text-sm sm:text-base font-semibold text-slate-700 mt-0.5 tracking-wide">Target Posisi</p>
          ...
          <div id="cv-section-certifications" class="mt-4">
            <h2 id="header-certifications" class="text-xs font-bold uppercase tracking-wider theme-accent-text border-b theme-accent-border pb-1 mb-1.5">Sertifikasi & Lisensi</h2>
  ...
  <script>
    ...
    // KAMUS EKSTENSIF TERJEMAHAN ID -> EN
    const advancedTranslationDict = [
      ...
      { id: /\bMeningkatkan\b/gi, en: "Increased and enhanced" },
      { id: /\bMengurangi\b/gi, en: "Reduced" },
      ...
      { id: /\bMembangun\b/gi, en: "Built and structured" },
      { id: /\bMerancang\b/gi, en: "Architected and designed" },
      { id: /\bMengeksekusi\b/gi, en: "Executed" },
      { id: /\bSarjana Teknik Industri\b/gi, en: "Bachelor of Industrial Engineering" },
      { id: /\bSarjana Teknik\b/gi, en: "Bachelor of Engineering" },
      { id: /\bSarjana Ekonomi\b/gi, en: "Bachelor of Economics" },
      ...
      { id: /\bFakultas\b/gi, en: "Faculty of" },
      { id: /\bJurusan\b/gi, en: "Department of" },
      { id: /\bper kuartal\b/gi, en: "per quarter" },
      { id: /\bper tahun\b/gi, en: "annually" },
      { id: /\bsetara\b/gi, en: "equivalent to" },
      ...
      { id: /\bdengan\b/gi, en: "with" },
      { id: /\buntuk\b/gi, en: "for" },
      { id: /\bdan\b/gi, en: "and" },
      { id: /\bserta\b/gi, en: "as well as" },
      { id: /\bdalam\b/gi, en: "in" },
      { id: /\bdari\b/gi, en: "from" },
      { id: /\bke\b/gi, en: "to" },
      { id: /\bhingga\b/gi, en: "up to" },
      { id: /\bsebesar\b/gi, en: "by" },
      { id: /\bsenilai\b/gi, en: "valued at" }
    ];
    ...
    let currentDashboardTab = 'active'; // 'active' or 'trash'
    ...
    function loadCvList() {
      ...
      if (raw) {
        try {
          cvList = JSON.parse(raw);
        } catch(e) { cvList = []; }
      } else {
        // Contoh awal jika kosong
        cvList = [createDefaultCvObject("CV Lead Operations (Eksekutif)")];
        saveCvList();
      }
      autoPurgeTrash();
    }
    ...
    function updateDashboardCounts() {
      const activeCount = cvList.filter(c => !c.deletedAt).length;
      const trashCount = cvList.filter(c => c.deletedAt).length;
      document.getElementById('badge-active-count').innerText = activeCount;
      document.getElementById('badge-trash-count').innerText = trashCount;
    }
          ...
          <div class="p-5 bg-slate-900/90 rounded-2xl border border-slate-800 hover:border-slate-700 transition flex flex-col justify-between shadow-lg">
            <div>
              <div class="flex items-start justify-between gap-2 mb-2">
                ...
                <span class="text-[10px] uppercase tracking-wider font-semibold px-2 py-0.5 rounded bg-blue-950 text-blue-300 border border-blue-800">ATS Ready</span>
              </div>
              <p class="text-xs text-slate-400 font-medium">${d.title || 'Target Posisi Belum Diisi'}</p>
            ...
            <div class="mt-5 pt-3 border-t border-slate-800 flex items-center justify-between gap-2">
              ${currentDashboardTab === 'active' ? `
                ...
                <div class="flex items-center space-x-1">
                  <button onclick="duplicateCV('${cv.id}')" title="Duplikasi CV" class="p-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs transition">
                    Duplikat
                  </button>
                  <button onclick="moveToTrash('${cv.id}')" title="Pindahkan ke Sampah" class="p-1.5 rounded-lg bg-rose-950/40 hover:bg-rose-900/60 text-rose-400 border border-rose-900/50 text-xs transition">
                    Hapus
                  </button>
                </div>
              ` : `
    ...
    function createDefaultCvObject(title = "CV Baru") {
      return {
        id: 'cv_' + Date.now() + '_' + Math.random().toString(36).substr(2, 5),
        title: title,
        updatedAt: Date.now(),
        deletedAt: null,
        data: {
          name: "Denyt Yudhanusa, S.T.",
          title: "Lead Operations & Logistics Ecosystem",
          email: "contact@gaeks.com",
          phone: "+62 812-3456-7890",
          location: "Jakarta, Indonesia",
          linkedin: "gaeks-executive",
          ...
          skills: "Freight Forwarding, Supply Chain, Customs Clearance, ERP Systems, Sea & Air Cargo, Negosiasi Kontrak",
          tools: "Dolibarr ERP, SAP, Excel Advanced, SQL, Cloudflare",
          certifications: "Sertifikasi Ahli Kepabeanan (PPJK)\nCertified Supply Chain Professional (CSCP)",
    ...
    function createNewCV() {
      const newCv = createDefaultCvObject("CV Saya " + (cvList.length + 1));
    ...
    function duplicateCV(id) {
      ...
      clone.id = 'cv_' + Date.now() + '_' + Math.random().toString(36).substr(2, 5);
      clone.title = source.title + " (Salinan)";
    ...
    // 2. EDITOR CONTROLLER & UNDO/REDO ENGINE
    ...
    function updateCvTitle(newTitle) {
    ...
    function handleInputChange() {
      ...
      // Record snapshot ke Undo Stack (Debounced / check equality)
    ...
    // AI TRANSLATE HELPER FIELD
    function aiTranslateField(elementId) {
    ...
    function setTheme(name) {
      ...
      ['navy', 'emerald', 'slate'].forEach(t => {
        ...
        if (b) {
          b.className = (t === name) 
            ? "px-2 py-1 rounded font-semibold bg-blue-950 text-blue-300 border border-blue-600"
            : "px-2 py-1 rounded font-semibold text-slate-400 hover:text-white";
        }
      });
      renderCV();
    }
    ...
    function addExperience(item = { role: '', company: '', period: '', location: '', desc: '' }) {
      ...
      div.className = "exp-item p-3 bg-slate-950 rounded-xl border border-slate-800 relative space-y-2";
      div.innerHTML = `
        <button onclick="this.parentElement.remove(); handleInputChange();" class="absolute top-2 right-2 text-xs text-rose-400">✕ Hapus</button>
        <div class="grid grid-cols-2 gap-2">
          <input type="text" placeholder="Jabatan" value="${item.role}" oninput="handleInputChange()" class="exp-role px-2.5 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded text-white" />
          <input type="text" placeholder="Perusahaan" value="${item.company}" oninput="handleInputChange()" class="exp-company px-2.5 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded text-white" />
        </div>
        <div class="grid grid-cols-2 gap-2">
          <input type="text" placeholder="Periode" value="${item.period}" oninput="handleInputChange()" class="exp-period px-2.5 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded text-white" />
          <input type="text" placeholder="Lokasi" value="${item.location}" oninput="handleInputChange()" class="exp-loc px-2.5 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded text-white" />
        </div>
        <textarea rows="3" placeholder="Pencapaian (tuliskan poin & angka)" oninput="handleInputChange()" class="exp-desc w-full px-2.5 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded text-white">${item.desc}</textarea>
      `;
    ...
    function addEducation(item = { degree: '', school: '', period: '', details: '' }) {
      ...
      const div = document.createElement('div');
      div.className = "edu-item p-3 bg-slate-950 rounded-xl border border-slate-800 relative space-y-2";
      div.innerHTML = `
        <button onclick="this.parentElement.remove(); handleInputChange();" class="absolute top-2 right-2 text-xs text-rose-400">✕ Hapus</button>
        <div class="grid grid-cols-2 gap-2">
          <input type="text" placeholder="Gelar" value="${item.degree}" oninput="handleInputChange()" class="edu-deg px-2.5 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded text-white" />
          <input type="text" placeholder="Kampus" value="${item.school}" oninput="handleInputChange()" class="edu-school px-2.5 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded text-white" />
        </div>
        <div class="grid grid-cols-2 gap-2">
          <input type="text" placeholder="Tahun" value="${item.period}" oninput="handleInputChange()" class="edu-period px-2.5 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded text-white" />
          <input type="text" placeholder="IPK" value="${item.details}" oninput="handleInputChange()" class="edu-det px-2.5 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded text-white" />
        </div>
      `;
      list.appendChild(div);
      handleInputChange();
        ...
        expView.innerHTML += `
          <div>
            <div class="flex flex-wrap justify-between items-baseline gap-1">
              <div>
                <span class="font-bold text-slate-900 text-xs">${applyTranslation(role)}</span>
                <span class="font-semibold text-slate-700 text-xs"> | ${comp}</span>
                ${loc ? `<span class="text-[11px] text-slate-500 italic"> (${loc})</span>` : ''}
              </div>
              <span class="theme-badge px-2 py-0.5 rounded text-[10px] font-semibold">${applyTranslation(per)}</span>
            </div>
...
git add cv.html
git commit -m "feat: implement CV manager dashboard, 3-day trash retention, undo-redo and advanced translation engine"
